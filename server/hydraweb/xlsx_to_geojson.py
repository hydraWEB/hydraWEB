...
import argparse
import re
def xlsxTOGeojson(fileName, read_dir, write_dir):
    read_dir_path = os.path.join(read_dir, fileName+'.xlsx')
    csv_dir_path = os.path.join(read_dir, 'temp_csv.csv')
    write_dir_path = os.path.join(write_dir, fileName+'.json')
    df = pd.read_excel(read_dir_path)
    df.to_csv(csv_dir_path, index=None, header=True)
    with open(csv_dir_path, 'r', newline='', encoding="utf8") as csvfile:
        reader=[]
