...
def GeojsonToShp(fileName, read_location, write_location):
    ...
    def create_shp_with_geojson(json,geo_type,dict1,name):
    ...
    with open(os.path.join(read_location,fileName),'r', encoding="UTF-8") as jsonfile:
        data = json.load(jsonfile)
        geo_type=str(data['features'][0]['geometry']['type'])
        fname = fileName.replace(".json", "")
        create_shp_with_geojson(data,geo_type,write_location,fname)
"""         new_dict=[]
        new_name=[]
...
test = GeojsonToShp("test", "/var/www/html/app-deploy/HydraWeb/python", "/var/www/html/app-deploy/HydraWeb/python")